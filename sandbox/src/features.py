...
class HarrisDetector:
    ...
    def _match_descriptors(
        self, query_descriptors, database_descriptors, match_lambda
    ) -> np.ndarray:
        """

        Returns:
            np.ndarray: (1, num_descriptors_in_query_image)
        """
        # shape: (Q, D) -- in this case (200, 200)
    ...
    def _describe_keypoints(self, img, keypoints, r) -> np.ndarray:
        N = keypoints.shape[1]
    ...
    def _find_keypoints(self, scores, num, r) -> np.ndarray:
        # keep `num` keypoints, each has 2 coordinates (u, v)
