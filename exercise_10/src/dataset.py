...
class IMUDataReader(object):
    def __init__(self, path, scaler, starttime=-float("inf")):
        ...
        self.path = path
        self.starttime = starttime
        self.field = namedtuple(
            "imu_msg", ["timestamp", "angular_velocity", "linear_acceleration"]
        )
...
class Stereo(object):
    def __init__(self, cam0, cam1):
        """Stereo images from two cameras"""
...
if __name__ == "__main__":
    ...
    print(
        f"dataset time interval: {timestamps[-1]} -> {timestamps[0]}"
