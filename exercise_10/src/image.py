...
class ImageProcessor(object):
    ...
    def __init__(self, config):
        ...
        # load config
        # Camera calibration parameters for the two stereo cameras.
    ...
    def prune_features(self):
        ...
        for i, features in enumerate(self.curr_features):
            ...
            if len(features) <= self.config.grid_max_feature_num:
                continue
            self.curr_features[i] = sorted(
                features, key=lambda x: x.lifetime, reverse=True
            )[: self.config.grid_max_feature_num]
    ...
    def stereo_match(self, cam0_points):
        ...
        norm_pixel_unit = 4.0 / (
            ...
            + self.cam0_intrinsics[1]
            + self.cam1_intrinsics[0]
            + self.cam1_intrinsics[1]
        )
    ...
    def undistort_points(
        ...
        distortion_model,
        distortion_coeffs,
        rectification_matrix=np.identity(3),
        ...
        if distortion_model == "equidistant":
            pts_out = cv2.fisheye.undistortPoints(
                pts_in, K, distortion_coeffs, rectification_matrix, K_new
            )
        else:  # default: 'radtan'
            pts_out = cv2.undistortPoints(
                pts_in, K, distortion_coeffs, None, rectification_matrix, K_new
            )
    ...
    def distort_points(self, pts_in, intrinsics, distortion_model, distortion_coeffs):
        ...
        if distortion_model == "equidistant":
            ...
            pts_out, _ = cv2.projectPoints(
                homogenous_pts, np.zeros(3), np.zeros(3), K, distortion_coeffs
            )
        return pts_out.reshape((-1, 2))

    def draw_features_stereo(self):
        """
        Draws the stereo features on the current image.
        """
