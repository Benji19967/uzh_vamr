...
for i in range(best_guess_history.shape[1]):
    guess_plot = axs[0].plot(
        x, np.polyval(best_guess_history[:, i], x), c="b", linewidth=1
    )

axs[0].plot(x, np.polyval(poly, x), "g", linewidth=2, label="ground truth")
axs[0].plot(
    x,
    np.polyval(best_guess_history[:, -1], x),
    "r",
    linewidth=2,
    label="RANSAC result",
)
...
axs[1].plot(
    matched_query_keypoints[1, np.logical_not(inlier_mask)],
    matched_query_keypoints[0, np.logical_not(inlier_mask)],
    "rx",
    linewidth=2,
)
axs[1].plot(
    matched_query_keypoints[1, inlier_mask],
    matched_query_keypoints[0, inlier_mask],
    "rx",
    linewidth=2,
)
plotMatches(
    corresponding_matches[inlier_mask],
    matched_query_keypoints[:, inlier_mask],
    keypoints,
    axs[1],
)
