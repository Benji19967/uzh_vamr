...
def runBA(hidden_state, observations, K):
    ...
    if with_pattern:
        ...
        pattern = scipy.sparse.lil_matrix(
            (num_error_terms, hidden_state.shape[0]), dtype=np.int8
        )
        ...
        for frame_i in range(num_frames):
            ...
            pattern[
                ...
                frame_i * 6 : (frame_i + 1) * 6,
            ] = 1

            # Each error is then also affected by the corresponding landmark.
            landmark_indices = observations[
                observation_i + 2 * num_keypoints_in_frame + 1 : observation_i
                + 3 * num_keypoints_in_frame
                + 1
            ]

            for kp_i in range(landmark_indices.shape[0]):
                pattern[
                    error_i + kp_i * 2 : error_i + (kp_i + 1) * 2,
                    num_frames * 6 + int(landmark_indices[kp_i] - 1) * 3 : num_frames
                    * 6
                    + int(landmark_indices[kp_i]) * 3,
                ] = 1
    ...
    def baError(hidden_state):
        ...
        for i in range(num_frames):
            ...
            keypoints = np.flipud(
                observations[
                    observation_i + 2 : observation_i + 2 + num_frame_observations * 2
                ]
                .reshape([-1, 2])
                .T
            )
            ...
            landmark_indices = observations[
                observation_i + 2 + num_frame_observations * 2 : observation_i
                + 2
                + num_frame_observations * 3
            ]
            ...
            p_C_L = (
                np.matmul(T_C_W[:3, :3], p_W_L.transpose(1, 0)[:, :, None]).squeeze(-1)
                + T_C_W[:3, -1]
            )
    ...
    res_1 = least_squares(
        baError, hidden_state, max_nfev=20, verbose=2, jac_sparsity=pattern
    )
