...
def main(
    ...
    if np.abs(rotation_img2_deg) > 1e-6:
        ...
        rotate_img = cv2.getRotationMatrix2D(
            (w_rot / 2, h_rot / 2), rotation_img2_deg, 1.0
        )
    ...
    for i in range(len(imgs)):
        ...
        desc, locs = computeDescriptors(
            blurred_images, tmp_keypoint_locs, rotation_invariant
        )
